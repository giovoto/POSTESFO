 ...
 * Crea las tablas necesarias SIN PostGIS
 */
...
const initDatabase = async () => {
  ...
  try {
    ...
    await client.query(`
      CREATE INDEX IF NOT EXISTS idx_postes_latitud 
      ON postes(latitud);
    `);
    await client.query(`
      CREATE INDEX IF NOT EXISTS idx_postes_longitud 
      ON postes(longitud);
    `);
    await client.query(`
      CREATE INDEX IF NOT EXISTS idx_postes_created_at 
      ON postes(created_at DESC);
    `);
    ...
    `);
    await client.query(`
      CREATE INDEX IF NOT EXISTS idx_fotos_poste_id 
      ON fotos(poste_id);
    `);
    ...
    console.log('   Email: admin@fiberoptic.com');
